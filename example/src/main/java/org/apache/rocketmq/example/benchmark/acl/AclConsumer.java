...
package org.apache.rocketmq.example.benchmark.acl;
...
public class AclConsumer {
    ...
    public static void main(String[] args) throws MQClientException, IOException {
        ...
        final String accessKey = commandLine.hasOption('a') ? commandLine.getOptionValue('a').trim() : null;
        final String secretKey = commandLine.hasOption('s') ? commandLine.getOptionValue('s').trim() : null;
        ...
        if (null != accessKey && null != secretKey) {
            System.out.printf("rocketmq acl enabled");
            rpcHook = new AclClientRPCHook(new SessionCredentials(accessKey, secretKey));
        }
        final DefaultMQProducer producer = rpcHook == null ? new DefaultMQProducer("benchmark_producer") : new DefaultMQProducer("benchmark_producer", rpcHook);

        DefaultMQPushConsumer consumer =  rpcHook == null ? new DefaultMQPushConsumer(group) : new DefaultMQPushConsumer(group, rpcHook,new AllocateMessageQueueAveragely());
    ...
    public static Options buildCommandlineOptions(final Options options) {
        ...
        opt = new Option("a", "accessKey", true, "access key");
        opt.setRequired(false);
        options.addOption(opt);

        opt = new Option("s", "secretKey", true, "secret key");
        opt.setRequired(false);
        options.addOption(opt);
        return options;
    }
